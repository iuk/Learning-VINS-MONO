...
void process() {
  while (true) {
    ...
    for (auto &measurement : measurements) {
      ...
      //遍历该img组中的各帧imus数据，进行预积分
      for (auto &imu_msg : measurement.first) {
        // imu 时间  最新IMU数据的时间戳
        double t = imu_msg->header.stamp.toSec();
        // img 时间  图像特征点数据的时间戳，补偿了通过优化得到的一个时间偏移
        double img_t = img_msg->header.stamp.toSec() + estimator.td;

        // 【通常情况】 如果 imu 时间 早于 img 时间
        ...
        if (t <= img_t) {
          ...
          // 这里对 IMU 数据进行 预积分 和 积分
          estimator.processIMU(dt, Vector3d(dx, dy, dz), Vector3d(rx, ry, rz));
          //printf("imu: dt:%f a: %f %f %f w: %f %f %f\n",dt, dx, dy, dz, rx, ry, rz);

        }
        // 【特殊情况】如果 imu 时间 晚于 img 时间
      ...
      if (relo_msg != NULL) {
        ...
        estimator.setReloFrame(frame_stamp,
                               frame_index,
                               match_points,
                               relo_t, relo_r);  // 设置回环帧
      }
      ...
      // 为什么键值是一个vector啊，一个id的特征点对应一个vector，难道是因为可能有多个相机
      // 建立每个特征点的(camera_id,[x,y,z,u,v,vx,vy])s的map，索引为feature_id
      map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> image;
      // 遍历 img 数据中的所有 point (归一化平面 x,y,1)
