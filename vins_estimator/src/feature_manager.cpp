...
/**
 * @brief 选取两帧之间共有的Features
 * @param frame_count_l
 * @param frame_count_r
 * @return 返回两帧之间的 3D 点对
 */
vector<pair<Vector3d, Vector3d>> FeatureManager::getCorresponding(int frame_count_l,
  ...
  for (auto &it : feature) 
  {
    ...
    if (it.start_frame <= frame_count_l &&
        it.endFrame() >= frame_count_r) {
...
double FeatureManager::compensatedParallax2(
    const FeaturePerId &it_per_id,  // 特征
    int frame_count) {
  //check the second last frame is keyframe or not
  //parallax betwwen seconde last frame and third last frame
  // 取出此特征点 在 第3 最新帧的 7 维信息
  const FeaturePerFrame &frame_i =
      it_per_id.feature_per_frame[frame_count - 2 - it_per_id.start_frame];  // 第3最新帧
  // 取出此特征点 在 第2 最新帧的 7 维信息
  const FeaturePerFrame &frame_j =
      it_per_id.feature_per_frame[frame_count - 1 - it_per_id.start_frame];  // 第2最新帧
  ...
  double dep_i = p_i(2);                  //1
  double u_i   = p_i(0) / dep_i;          // 归一化平面 x
  double v_i   = p_i(1) / dep_i;          // 归一化平面 y
  double du = u_i - u_j, dv = v_i - v_j;  // 归一化平面 du dv
