...
class IntegrationBase {
 public:
  ...
  void midPointIntegration(
      // input
      double _dt,
      const Eigen::Vector3d &_acc_0,    // 上一次传感器值
      const Eigen::Vector3d &_gyr_0,
      const Eigen::Vector3d &_acc_1,    // 本次传感器值
      const Eigen::Vector3d &_gyr_1,
      const Eigen::Vector3d &delta_p,   // 上次预积分结果
      const Eigen::Quaterniond &delta_q,
      const Eigen::Vector3d &delta_v,
      const Eigen::Vector3d &linearized_ba, // 上次预积分的 bias
      const Eigen::Vector3d &linearized_bg,
      // output
      Eigen::Vector3d &result_delta_p,  // 本次预积分结果
      Eigen::Quaterniond &result_delta_q,
      Eigen::Vector3d &result_delta_v,
      Eigen::Vector3d &result_linearized_ba,    // 本次预积分 bias，并不会发生改变
      Eigen::Vector3d &result_linearized_bg,
      bool update_jacobian) {
    //ROS_INFO("midpoint integration");
    // 上帧系下的, 去偏置的， 三轴加速度 0，因为重力加速度和绝对姿态有关，这里没有去除重力加速度。
    Vector3d un_acc_0    = delta_q * (_acc_0 - linearized_ba);
    // 本帧系下的，去偏置的，中值三轴角速度
    Vector3d un_gyr      = 0.5 * (_gyr_0 + _gyr_1) - linearized_bg;
    // 上帧系下的本帧的四元数姿态积分 q_k+1 = q_k ⊗ [1,½ωΔt]
