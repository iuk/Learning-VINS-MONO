  ...
  if (PUB_THIS_FRAME) {
    ...
    if (n_max_cnt > 0) {
      ...
      /** 
             ...
             *)   
             */
      // goodFeaturesToTrack() 获取跟踪角点到 n_pts 中
...
void FeatureTracker::rejectWithF() {
  if (forw_pts.size() >= 8) {
    ...
    // 定义 un_cur_pts un_forw_pts
    vector<cv::Point2f> un_cur_pts(cur_pts.size()), un_forw_pts(forw_pts.size());

    // 遍历光流前一帧所有的 跟踪点
    for (unsigned int i = 0; i < cur_pts.size(); i++) {
      Eigen::Vector3d tmp_p;  // 单位圆上的点
      //根据不同的相机模型将二维坐标转换到三维坐标(反向投影到单位圆上)(不是归一化平面)
      m_camera->liftProjective(Eigen::Vector2d(cur_pts[i].x, cur_pts[i].y), tmp_p);
      //空间点(单位圆)转到一个 (假设的?) 像素平面上
...
void FeatureTracker::undistortedPoints() {
  ...
  // 与 prev 帧同id特征点作比较速度单位为归一化平面上的m
